    ...
    <main className="mx-auto max-w-[820px] px-4 py-12 md:py-16">
      ...
      <section className="mt-8 space-y-4 text-sm leading-7 text-gray-800">
        ...
        <p>
          ...
          전송되지 않고 이용자의 브라우저 안에서만 계산됩니다. 다만 &lsquo;계약서 파일로 자동 입력&rsquo;
          기능을 이용하는 경우에 한해 업로드한 파일이 판독(분석) 목적으로만 서버를 거쳐 인공지능
          모델에 전달되며, 파일과 판독 결과는 저장하지 않고 응답 후 즉시 폐기합니다. 판독 항목은 계약
          조건(기간·근로시간·임금·조항)에 한정하며 성명·생년월일·주민등록번호·주소·연락처는 수집하지
          않습니다. 다만 <strong>파일은 통째로 전송되므로</strong>, 업로드 전에 이름·주민등록번호·주소 등 판독에 불필요한 개인정보는 가려 주시기 바랍니다. 파일 자동 입력 기능을 이용하지 않으면 어떤 계약 정보도 서버로 전송되지 않습니다.
        </p>
        <p>
        ...
        <p>
          · <strong>Anthropic PBC</strong>(미국) — 계약서 파일 판독. 이전 항목은 이용자가 업로드한
          계약서 파일이며, 이전 시점은 파일 업로드 시, 보유 기간은 판독 응답 직후까지입니다.<br />
          · <strong>Cloudflare, Inc.</strong>(미국) — 자동화(봇) 차단. 이전 항목은 접속 IP와 브라우저
          신호이며, <strong>파일을 올리지 않아도 해당 화면에 접속하는 것만으로</strong> 확인 절차가
          동작합니다.<br />
          · <strong>Google LLC</strong>(미국) — 웹사이트 이용 통계 분석.
        </p>
        <p>
          이용자는 위 처리위탁·국외 이전에 동의하지 않을 수 있으며, 이 경우 파일 자동 입력 기능을
