-- ★ 사용하지 않음 (2026-08-01). 라우트는 챗과 같은 공용 `incr_rate_limit`을 쓰도록 바뀌었다.
--   전용 RPC를 따로 만들었더니 프로덕션에서 조용히 fail-open 됐고(마이그레이션이 어느 프로젝트에
--   적용됐는지 밖에서 확인이 안 됨), 이미 검증된 공용 카운터를 재사용하는 편이 안전했다.
--   적용해도 무해하지만 새로 적용할 필요는 없다.
--
