> # ⛔ 폐기됨 — `DESIGN.md`로 대체됨. 참조 금지.
>
> **이 문서는 디자인 정본이 아니다.** 이 레포의 디자인 정본은 [`DESIGN.md`](./DESIGN.md) 하나뿐이다.
> 아래 내용은 이력 보존을 위해 남겨 둔 것이며, **여기 적힌 값·규격을 코드에 반영하지 마라.**
>
> - 두 문서를 **병합하지 마라** (DESIGN.md §0 머리말).
> - 현행과 어긋나는 구체적 지점은 **DESIGN.md §0.2**에 목록으로 정리돼 있다.
> - 특히 이 문서가 규정했던 **콘텐츠 블록 5종은 폐기**되었고, 산문 조판 규격은 **DESIGN.md §6.8**이 대체한다.
>
> 신규 작업은 전부 `DESIGN.md`를 근거로 한다. (§9 P6-3)

---

# 노동법 인사이트 — 디자인 & 개발 가이드 v2 (폐기)
